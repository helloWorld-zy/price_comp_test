...
import type { Supplier, PaginatedResult, ApiError } from '@/api/client'
...
const loadItems = async () => {
  ...
  try {
    const result: PaginatedResult<Supplier> = await supplierApi.list({
...
const openCreateModal = () => {
  ...
  form.value = { name: '', contact: '', aliases: '', status: 'ACTIVE' }
...
const handleSave = async () => {
  ...
  try {
    ...
    if (modalMode.value === 'create') {
      ...
      await supplierApi.update(editingItem.value.id, data)
    }
...
const openDeleteModal = (item: Supplier) => {
...
const handleDelete = async () => {
  ...
  try {
    await supplierApi.delete(deletingItem.value.id)
    closeDeleteModal()
    await loadItems()
  } catch (err) {
    const apiError = err as ApiError
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ modalMode === 'create' ? '添加供应商' : '编辑供应商' }}</h5>
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <input v-model="form.name" type="text" class="form-control" placeholder="例如：携程旅游" />
            </div>
            <div class="mb-3">
              <label class="form-label">联系方式</label>
              <input v-model="form.contact" type="text" class="form-control" placeholder="电话或邮箱" />
            </div>
            <div class="mb-3">
    ...
    <div v-if="showDeleteModal" class="modal fade show d-block" tabindex="-1" style="background: rgba(0,0,0,0.5)">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>确定要删除供应商 <strong>{{ deletingItem?.name }}</strong> 吗？此操作不可撤销。</p>
          </div>
          <div class="modal-footer">
